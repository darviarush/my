<?php
/*
	Dist: https://code.google.com/p/test-more-php
	Author: beyondlogical@gmail.com
...
class TestSimple {
	...
	#	if ($this->NumberOfTests === 'skip_all') plan(0);
		...
		if ($this->TestsRun && !isset($this->NumberOfTests)) {
			fwrite(STDERR, "# Tests were run but no plan() was declared and done_testing() was not seen.\n");
		} else {
			if ($this->TestsRun !== $this->NumberOfTests) fwrite(STDERR, "# Looks like you planned ".(int)$this->NumberOfTests .' tests but ran '.(int)$this->TestsRun.".\n");
	
			if ($this->Results['Failed']) fwrite(STDERR, "# Looks like you failed ".  $this->Results['Failed'] .' tests of '.(int)$this->TestsRun.".\n");
		}
